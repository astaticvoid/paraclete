...
**If you are starting an implementation session, read `design/handoff.md` first** — it routes tasks by model tier and carries the guardrails. Current sequence (authority: `design/roadmap.md`): ~~P10 C0 / W0 / P10 C1 / W1 C0–C4 / paired session #1~~ all shipped (see roadmap history) → ~~Theoria legibility phase~~ **shipped 2026-07-10** (`design/phases/theoria-legibility-report.md`: labeled track-select, visible mode, grid labels, topology-driven tracks, contextual encoders; BUG-016…021 found+fixed; Antiphon **open by default**, `--token` opt-in; USB-C direct tablet link verified at 3.0 ms RTT) → ~~paired session #2~~ **held 2026-07-10 on glass** (`design/sessions/s2.md`; verdict: new baseline, but the Launchpad-grid mirror is the wrong foundation — target = **consistent fixed inputs + contextual screen window**; Launchpad parked; BUG-022 fixed `5071c9a`, BUG-023 engine exonerated by measurement, awaiting driver headphone A/B) → **next: (a) Theoria baseline interaction wins** (drag-draw steps, encoder gesture/placement, hide dead grid — solo), **(b) P10 C2+ pattern depth** (spec'd, solo), **(c) W2 re-scoped reference-design spike** (paired: Digitakt II + Syntakt + Hydrasynth manuals → native-surface spec + machine-family param philosophy; ADR-032 after). Voice depth (P12+/P13) is the recorded emotional core ("dial from scratch") — spec it from the reference spike, don't improvise. BUG-012 still queued for a hardware session.
