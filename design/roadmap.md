...
>
> **Last updated:** 2026-07-27 (latest). **TK2.1 redesign pass drafted —
...
| SPIKE-006 | **What can the LaunchControl XL actually give us?** | It has **pots, not encoders** — absolute positions, so it cannot drive the relative-only encoder contract without soft-takeover, and it cannot follow a contextual page without value jumps. The useful shape is fixed macro bindings (W5 + P16), not a contextual bank. Open beyond that: does its own control-surface mode expose anything better; what do its templates/factory modes actually transmit; is there **prior art for firmware modification** on this device (community firmware, open bootloader, CC-mode hacks) and would a modified firmware transmit deltas; if not, what does a true relative-encoder controller cost and which ones actually implement relative modes | Bench it: enumerate what each mode transmits, and whether soft-takeover in a profile makes pots tolerable for live parameter moves. Survey prior art on XL firmware modification. Outcome is a paragraph in W5's spike and, if firmware modification looks real, its own ADR. **Do not** re-explore the Digitakt as a controller — settled 2026-07-04, absolute-only |
