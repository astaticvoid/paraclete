...
| `Cow<'static, str>` L2 migration (kill `Box::leak` in `paraclete-clap-host/bridge.rs`) — see Known Provisional table (D6); **audit U1** | **Before `paraclete-node-api` v0.1.0 / first crates.io publish** — free until then, breaking after. **Reclassified 2026-07-12 (universality audit U1): also W2-blocking** — dynamic per-client Theoria surfaces need runtime names, so pull it into the W2 surface work, not the pre-publication pass. Full site list in `design/review/universality-audit.md`. |
