...
| 4c | **MM**: ADR-041 + ADR-042 impl (machine select, MOD page) — `mm-machine-and-mod.md` | **Code-complete** (C0–C11, 2026-08-01). ADR-041/042 marked implemented. Exit criteria §6.1–§6.6 met; **§6.7's session gate is the only thing outstanding** — machine switch and LFO depth are both performance gestures, and TRIG now sits at page index 0 ahead of SRC, so page keys 1-6 select different pages than a performer has learned. Milestone stays open until that session. Post-MM defect pass 2026-08-01 closed #152 (wrong-track edits) and #157 (`view_meta` froze at the built-with machine) before that session, so its findings are about the design and not about two known defects; #161 (the machine's *name* is still the built-with one, panel header included) is open and performer-visible, worth landing first. |
