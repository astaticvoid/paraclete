...
| **AN** | Anamnesis sampling layer | Capture-to-performance loop: HAL input + recorder rings (retroactive capture, resampling), app-owned sample pool + per-step sample locks, slices/chains, scenes + crossfader morph, pickup-style looping, staged timestretch — AN0–AN3, session-gated | **Track drafted 2026-07-23** (`design/sampling/{problem,design}.md`; ADR-040 🟡 proposed). AN2 scenes depend on P11 KitStore; scheduling decided at TK2 exit |
