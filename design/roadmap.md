...
| 4c | **MM**: ADR-041 + ADR-042 impl (machine select, MOD page) — `mm-machine-and-mod.md` | **Code-complete** (C0–C11, 2026-08-01). ADR-041/042 marked implemented. Exit criteria §6.1–§6.6 met; **§6.7's session gate is the only thing outstanding** — machine switch and LFO depth are both performance gestures, and TRIG now sits at page index 0 ahead of SRC, so page keys 1-6 select different pages than a performer has learned. Milestone stays open until that session. |
