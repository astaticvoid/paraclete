...
//! Paraclete FM Bass Machine — CLAP instrument (FmEngine::bass + Sequencer).
//!
//! Build with: cargo build -p paraclete-machine-fm-bass
//! Output: target/debug/libparaclete_machine_fm_bass.dylib (rename to .clap)
...
static PLUGIN_ID: &[u8] = b"audio.paraclete.machine.fm_bass\0";
static PLUGIN_NAME: &[u8] = b"Paraclete FM Bass\0";
static VENDOR: &[u8] = b"Paraclete Audio\0";
static VERSION: &[u8] = b"0.1.0\0";
static DESC: &[u8] = b"FM bass synthesizer with step sequencer\0";
...
unsafe extern "C" fn factory_create_plugin(
    ...
    let inner = SubgraphPlugin::new(Box::new(FmEngine::bass()), 3, 48000.0, 512);
