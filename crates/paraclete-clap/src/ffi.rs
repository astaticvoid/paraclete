...
use clap_sys::events::{
    clap_event_note, clap_event_param_value, CLAP_EVENT_NOTE_OFF, CLAP_EVENT_NOTE_ON,
            ...
            match hdr.type_ {
                CLAP_EVENT_NOTE_ON => {
                    ...
                    use paraclete_node_api::midi::{
                        u4, u7, ChannelVoice2, Channeled, Grouped, NoteOn,
                    };
                    ...
                    external_events.push(TimedEvent::new(hdr.time, Event::Midi2(ump)));
                }
                CLAP_EVENT_NOTE_OFF => {
                    let ev = &*(raw as *const clap_event_note);
                    use paraclete_node_api::midi::{
                        u4, u7, ChannelVoice2, Channeled, Grouped, NoteOff,
                    };
                    use paraclete_node_api::{Event, TimedEvent, UmpMessage};
                ...
                CLAP_EVENT_PARAM_VALUE => {
                    ...
                    if let Some(cmd) = w.inner.bridge().make_set_param_command(
                        ev.param_id,
                        ev.value,
                        w.inner.gen_id(),
                    ) {
