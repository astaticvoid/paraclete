...
use paraclete_clap::transport::{
    translate_transport, CLAP_TRANSPORT_HAS_BEATS_TIMELINE, CLAP_TRANSPORT_IS_PLAYING,
};
...
fn bridge_make_set_param_command_correct() {
    ...
    let cmd = bridge
        .make_set_param_command(0, 1200.0, 42)
...
fn translate_transport_start_transition_emits_global_start() {
    ...
    assert!(
        info.playing,
        "info.playing should be true when IS_PLAYING flag is set"
    );
    ...
    assert!(
        ...
        "global_stop should be false on play-start"
    );
}
...
fn translate_transport_stop_transition_emits_global_stop() {
    ...
    assert!(
        !info.playing,
        "info.playing should be false when IS_PLAYING is not set"
    );
...
fn machine_bank_ffi_entry_not_null() {
    ...
    assert!(
        vtable.activate.is_some(),
        "plugin.activate must be non-null"
    );
    assert!(
        vtable.deactivate.is_some(),
        "plugin.deactivate must be non-null"
    );
    assert!(
        vtable.start_processing.is_some(),
        "plugin.start_processing must be non-null"
    );
    assert!(
        vtable.stop_processing.is_some(),
        "plugin.stop_processing must be non-null"
    );
    assert!(vtable.reset.is_some(), "plugin.reset must be non-null");
    assert!(vtable.process.is_some(), "plugin.process must be non-null");
    assert!(
        vtable.get_extension.is_some(),
        "plugin.get_extension must be non-null"
    );
    assert!(
        vtable.on_main_thread.is_some(),
        "plugin.on_main_thread must be non-null"
    );
}
