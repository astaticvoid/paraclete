...
//! Paraclete FM Bell Machine — CLAP instrument (FmEngine::bell + Sequencer).
//!
//! Build with: cargo build -p paraclete-machine-fm-bell
//! Output: target/debug/libparaclete_machine_fm_bell.dylib (rename to .clap)

use clap_sys::factory::plugin_factory::clap_plugin_factory;
use clap_sys::host::clap_host;
use clap_sys::plugin::{clap_plugin, clap_plugin_descriptor};
use paraclete_clap::{ffi::*, SubgraphPlugin};
use paraclete_nodes::FmEngine;
use std::ffi::{c_char, c_void, CStr};

static PLUGIN_ID: &[u8] = b"audio.paraclete.machine.fm_bell\0";
static PLUGIN_NAME: &[u8] = b"Paraclete FM Bell\0";
static VENDOR: &[u8] = b"Paraclete Audio\0";
static VERSION: &[u8] = b"0.1.0\0";
static DESC: &[u8] = b"FM bell synthesizer with step sequencer\0";
...
static FEATURES: SyncFeatures<3> = SyncFeatures([
    ...
    clap_sys::plugin_features::CLAP_PLUGIN_FEATURE_SYNTHESIZER.as_ptr() as *const c_char,
    std::ptr::null(),
]);
...
unsafe extern "C" fn factory_create_plugin(
    ...
    let inner = SubgraphPlugin::new(Box::new(FmEngine::bell()), 3, 48000.0, 512);
