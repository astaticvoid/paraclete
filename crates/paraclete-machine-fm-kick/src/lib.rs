...
//! Paraclete FM Kick Machine — CLAP instrument (FmEngine::kick + Sequencer).
//!
//! Build with: cargo build -p paraclete-machine-fm-kick
//! Output: target/debug/libparaclete_machine_fm_kick.dylib (rename to .clap)
...
static PLUGIN_ID: &[u8] = b"audio.paraclete.machine.fm_kick\0";
static PLUGIN_NAME: &[u8] = b"Paraclete FM Kick\0";
static VENDOR: &[u8] = b"Paraclete Audio\0";
static VERSION: &[u8] = b"0.1.0\0";
static DESC: &[u8] = b"FM kick drum with step sequencer\0";
...
unsafe extern "C" fn factory_create_plugin(
    ...
    let inner = SubgraphPlugin::new(Box::new(FmEngine::kick()), 3, 48000.0, 512);
