...
//! Runtime performance counters shared between the audio thread and the main thread.
//!
//! All fields are `AtomicU64` so the audio thread can write and the main thread
//! can read without locks. Published to the state bus as `/engine/*` paths.

use std::sync::atomic::AtomicU64;

use crate::StateBusValue;
...
pub struct RuntimeCounters {
    ...
    /// EWMA of `process()` elapsed time in microseconds.
