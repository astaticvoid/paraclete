...
impl PluginNode {
    pub(crate) fn new(
        ...
        let mut port_list = vec![PortDescriptor {
            id: PORT_EVENTS_IN,
    ...
    pub fn bridge(&self) -> &HostParamBridge {
        &self.bridge
    }
}
...
impl Node for PluginNode {
    ...
    fn process(&mut self, input: &ProcessInput, output: &mut ProcessOutput) {
        ...
        for (i, entry) in self.bridge.entries.iter().enumerate() {
            let Some(flushed) = self.flushed_values.get_mut(i) else {
                continue;
            };
        ...
        for timed in input.events {
            if self.note_buf.len() >= MAX_EVENTS {
                break;
            }
            if let Event::Midi2(ref ump) = timed.event {
                if let Some(ev) = ump_to_note_event(timed.sample_offset, ump) {
                    ...
                    let hdr_ptr = unsafe {
                        &(*self.note_buf.as_ptr().add(idx)).header as *const clap_event_header
                    };
                    self.ptr_idx.push(hdr_ptr);
                }
            }
