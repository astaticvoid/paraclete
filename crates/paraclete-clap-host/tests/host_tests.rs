use paraclete_clap_host::{scan_clap_paths, HostParamBridge};
...
fn host_param_bridge_from_raw_params_round_trips() {
    ...
    assert_eq!(
        bridge.paraclete_id_for(decay_clap_id),
        Some(decay_paraclete_id)
    );
    assert_eq!(
        bridge.paraclete_id_for(cutoff_clap_id),
        Some(cutoff_paraclete_id)
    );
    assert_eq!(bridge.clap_id_for(decay_paraclete_id), Some(decay_clap_id));
    assert_eq!(
        bridge.clap_id_for(cutoff_paraclete_id),
        Some(cutoff_clap_id)
    );
}
...
fn plugin_node_from_kick_clap_produces_audio() {
    ...
    let mut node = lib
        .instantiate(&plugin_id, 48000.0, 512)
        .expect("instantiate");
    ...
    for block in 0..4 {
        ...
        let mut output = ProcessOutput::new(&mut audio_outs, &mut sig_outs, &mut out_events);
    ...
    assert!(
        max_val > 1e-5,
        "expected non-silent audio, got max={max_val}"
    );
}
...
fn plugin_node_capability_document_has_params() {
    ...
    assert!(
        !doc.params.is_empty(),
        "kick should expose at least one parameter"
    );
