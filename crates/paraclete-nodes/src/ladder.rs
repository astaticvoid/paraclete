...
impl LadderFilterNode {
    ...
    fn default_doc() -> CapabilityDocument {
        CapabilityDocument {
            ...
            ports: vec![],
            params: vec![
                ParamDescriptor { id: lad("cutoff"),    name: "cutoff".into(),    min: 20.0, max: 18000.0, default: 4000.0, stepped: false, in_kit: true,  unit: ParamUnit::Hz,      display: None },
                ParamDescriptor { id: lad("resonance"), name: "resonance".into(), min: 0.0,  max: 1.0,    default: 0.0,    stepped: false, in_kit: true,  unit: ParamUnit::Generic, display: None },
                ParamDescriptor { id: lad("drive"),     name: "drive".into(),     min: 0.0,  max: 1.0,    default: 0.0,    stepped: false, in_kit: true,  unit: ParamUnit::Generic, display: None },
            ],
...
mod tests {
    ...
    fn run_ladder(filt: &mut LadderFilterNode, input_signal: &[f32]) -> Vec<f32> {
        ...
        filt.process(&input,     &mut ProcessOutput::new(
            &mut outs,
            &mut [],
            &mut events_out,
        ));
