...
fn ts(start: Instant) -> f64 { start.elapsed().as_secs_f64() }

fn log(f: &Arc<Mutex<std::fs::File>>, start: Instant, tag: &str, msg: &str) {
    let line = format!("[{:6.2}s] {:12} {}\n", ts(start), tag, msg);
    if let Ok(mut g) = f.lock() { g.write_all(line.as_bytes()).ok(); g.flush().ok(); }
    eprint!("{}", line);
}
...
fn find_out(o: &midir::MidiOutput, s: &str) -> Option<midir::MidiOutputPort> {
    o.ports().into_iter().find(|p| o.port_name(p).map(|n| n.contains(s)).unwrap_or(false))
}
fn find_in(i: &midir::MidiInput, s: &str) -> Option<midir::MidiInputPort> {
    i.ports().into_iter().find(|p| i.port_name(p).map(|n| n.contains(s)).unwrap_or(false))
}
...
fn mode_name(b: u8) -> &'static str { if b == 1 { "Programmer" } else { "Live" } }
...
fn probe(
    ...
    let ls = layout.map(|b| format!("{:#04X}={}", b, layout_name(b)))
                   .unwrap_or_else(|| "TIMEOUT".into());
    let ms = mode.map(|b| format!("{}={}", b, mode_name(b)))
                 .unwrap_or_else(|| "TIMEOUT".into());

    log(log_f, start, "PROBE", &format!("{label:30} layout={ls:25} mode={ms}"));
}
...
fn cleanup(out: &mut midir::MidiOutputConnection,
           log_f: &Arc<Mutex<std::fs::File>>, start: Instant) {
    log(log_f, start, "CLEANUP", "reverting LP → Standalone + NoteMode");
...
fn main() {
    ...
    let log_f: Arc<Mutex<std::fs::File>> = Arc::new(Mutex::new(
        std::fs::OpenOptions::new().create(true).truncate(true).write(true)
            .open("/tmp/lpx-debug.log").unwrap()
    ));
    log(&log_f, start, "INIT", "lpx-debug starting — log: /tmp/lpx-debug.log");
    ...
    log(&log_f, start, "STEP-0", "reset: DAW→Standalone + NoteMode via DAW port");
    {
    ...
    if !wait(500, &running) { return; }

    // ── Open MIDI I/O ─────────────────────────────────────────────────────────
    log(&log_f, start, "STEP-1", "opening MIDI output");
    let o = midir::MidiOutput::new("lpx-out").unwrap();
    let op = find_out(&o, "LPX MIDI").or_else(|| find_out(&o, "Launchpad")).unwrap();
    log(&log_f, start, "STEP-1", &format!("out: {:?}", o.port_name(&op)));
    ...
    let ip = find_in(&i, "LPX MIDI").or_else(|| find_in(&i, "Launchpad")).unwrap();
    log(&log_f, start, "STEP-1", &format!("in: {:?}", i.port_name(&ip)));
    let _ci = i.connect(&ip, "i", move |_, b, _| {
        if b.is_empty() || b[0] == 0xF8 { return; }
        let h = b.iter().map(|x| format!("{x:02X}")).collect::<Vec<_>>().join(" ");
        log(&lf2, start2, "LP-IN", &h);
        ib.lock().unwrap().push(b.to_vec());
    }, ()).unwrap();

    if !wait(200, &running) { cleanup(&mut conn_out, &log_f, start); return; }
    probe(&mut conn_out, &inbox, &log_f, start, "after-connections-open");
    if !wait(2000, &running) { cleanup(&mut conn_out, &log_f, start); return; }
    ...
    if !wait(200, &running) { cleanup(&mut conn_out, &log_f, start); return; }
    probe(&mut conn_out, &inbox, &log_f, start, "after-programmer-sysex");
    if !wait(500, &running) { cleanup(&mut conn_out, &log_f, start); return; }
    ...
    log(&log_f, start, "STEP-1", "observing 3s (expect: red pad, no march)");
    if !wait(3000, &running) { cleanup(&mut conn_out, &log_f, start); return; }
    ...
    log(&log_f, start, "STEP-2", &format!("device: {:?}", device.name()));
    let cfg = device.default_output_config().unwrap();
    log(&log_f, start, "STEP-2", &format!("config: {cfg:?}"));
    let stream = device.build_output_stream(
        &cfg.into(), |d: &mut [f32], _| { d.fill(0.0); },
        |e| eprintln!("audio err: {e}"), None,
    ).unwrap();
    stream.play().unwrap();
    log(&log_f, start, "STEP-2", "audio stream playing");
    if !wait(100, &running) { cleanup(&mut conn_out, &log_f, start); return; }
    probe(&mut conn_out, &inbox, &log_f, start, "immediately-after-audio-start");
    log(&log_f, start, "STEP-2", "observing 5s (does audio trigger march?)");
    if !wait(5000, &running) { cleanup(&mut conn_out, &log_f, start); return; }
